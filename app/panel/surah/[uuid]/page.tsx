...
interface SimpleSurah {
    ...
    ayahs: { number: number, uuid: string, sajdeh: string, content: { text: string } }[];
}

async function getSurah(uuid: string): Promise<SimpleSurah> {
    const response = await fetch(`${process.env.API_URL}/surah/${uuid}?mushaf=hafs`);

    return response.json();
}

export default async function ViewSurah({ params }: { params: { uuid: string } }) {
    ...
    return (
        <Container>
            ...
            <Xtable data={singleSurah.ayahs.map(ayah => ({ number: ayah.number, text: ayah.content.text, sajdeh: ayah.sajdeh, uuid: ayah.uuid }))} />
        </Container>
    );
}
